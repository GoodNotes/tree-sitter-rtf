...
module.exports = grammar({
  ...
  rules: {
    document: ($) =>
      seq("{", $._document_header, optional($._document_area), "}"),
    ...
    // FONT TABLE
    fonttbl: ($) =>
      seq("{\\fonttbl", repeat(choice($.fontinfo, $._fontinfo_alt)), "}"),
    ...
    fontinfo: ($) =>
      seq(
        ...
        /\\f\d+/,
        $._fontFamilyBloq,
        optional(seq("\\fcharset", field("charset", $.charset))),
        optional(/\\fprq\d+/),
        optional($._panose),
    ...
    _fontFamilyBloq: ($) => seq("\\f", field("fontFamily", $.fontFamily)),
    ...
    _fontfname: ($) =>
      seq("{\\*", "\\fontfile", /\\cpg\d+/, $._static_PCDATA, "}"),

    // FILE TABLE
    _filetbl: ($) =>
      seq("{\\*", "\\filetbl", repeat1(seq("{", $._fileinfo, "}")), "}"),
    ...
    // COLOR TABLE
    colortbl: ($) => seq("{\\colortbl", repeat1($._colordef), "}"),
    ...
    colorvalue: ($) =>
      seq(
        "\\red",
        field("red", $.staticNumberLiteral),
        "\\green",
        field("green", $.staticNumberLiteral),
        "\\blue",
        field("blue", $.staticNumberLiteral)
      ),
    ...
    _bookend: ($) => seq("{\\*", "\\bkmkend", $._static_PCDATA, "}"),

    // DOCUMENT AREA => FROM HERE EVERYTHING IS CUSTOM
    _document_area: ($) => seq(repeat1($._repeatDocument_area)),

    _repeatDocument_area: ($) =>
      seq(
        optional($._paperTbl),
    ...
    _formatingProperties: () => repeat1(/\\deftab\d+/),

    _paperTbl: ($) => seq(repeat1($._paper_config)),
    ...
    _textUnitEmoji: ($) => prec(2, seq(repeat1($._unicodeBlock))),

    _unicodeBlock: ($) =>
      seq(optional(/\\uc\d+/), "\\u", field("unicode", $.unicode), " "),

    _textSpecialChar: ($) => prec(2, seq(repeat1($._specialCharBlock))),

    _specialCharBlock: ($) => seq("\\'", field("specialChar", $.specialChar)),
    ...
    italicDisabled: () => /\\i0/,
    underlineEnabled: () => "\\ul",
    underlineDisabled: () => /\\ul0|\\ulnone/,
    _underlineColorIndex: () => /\\ulc\d+/,
    strikeEnabled: () => "\\strike",
    strikeDisabled: () => "\\striked0",
